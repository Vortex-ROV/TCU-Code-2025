...
from VortexFloat import DepthPlotter
...
class VortexMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # FloatPage Button Logic
        self.floatButton.clicked.connect(lambda: self.mainBodyStackedWidget.setCurrentWidget(self.floatPage)) #-> to be edited

        self.floatPageLogic() #Float -> Station
        self.profileTabs.setTabText(0,"Profile 1")
        self.profileTabs.setTabText(1,"Profile 2")

    # ######################### Float Logic #################################
    def floatPageLogic(self):
        # Setting up the first graph display layout
        self.horizontalLayout_graphDisplay_1 = QtWidgets.QHBoxLayout(self.graphDisplay1)
        self.horizontalLayout_graphDisplay_1.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout_graphDisplay_1.setSpacing(0)

        # Setting up the second graph display layout
        self.horizontalLayout_graphDisplay_2 = QtWidgets.QHBoxLayout(self.graphDisplay2)
        self.horizontalLayout_graphDisplay_2.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout_graphDisplay_2.setSpacing(0)

        # Create separate figures and canvases for two graphs
        ...
        # Add each canvas to its respective layout
        self.horizontalLayout_graphDisplay_1.addWidget(self.canvas1)
        self.horizontalLayout_graphDisplay_2.addWidget(self.canvas2)

        # Define two separate plotter objects
        self.plotter1 = DepthPlotter(port='COM8', baudrate=115200)
        self.plotter2 = DepthPlotter(port='COM8', baudrate=115200)  # Assuming another COM port
        
        # Call detection function periodically
        self.detectionTimer = QtCore.QTimer(self)
        self.detectionTimer.timeout.connect(self.check_connection_and_detect)
        self.detectionTimer.start(5000)  # Call every 5 seconds

        # Connect buttons to start and stop functions
        self.startReading1.clicked.connect(lambda: self.start_reading(self.plotter1, self.canvas1))
        self.stopReading1.clicked.connect(lambda: self.stop_reading(self.plotter1, self.canvas1))

        self.startReading2.clicked.connect(lambda: self.start_reading(self.plotter2, self.canvas2))
        self.stopReading2.clicked.connect(lambda: self.stop_reading(self.plotter2, self.canvas2))

        # Timer for updating plots
        self.timer1 = QtCore.QTimer(self)
        self.timer1.timeout.connect(lambda: self.update_plot(self.plotter1, self.canvas1))

        self.timer2 = QtCore.QTimer(self)
        self.timer2.timeout.connect(lambda: self.update_plot(self.plotter2, self.canvas2))

    def check_connection_and_detect(self):
        if self.plotter1.is_connected():
            self.plotter1.detection()
        else:
            print("Connection is not occupied or disconnected.")

    def start_reading(self, plotter, canvas):
        plotter.read_data()
        if plotter == self.plotter1:
            self.timer1.start(1000)  # Update plot every second
        else:
            self.timer2.start(1000)

    def stop_reading(self, plotter, canvas):
        try:
            ...
            if plotter == self.plotter1:
                ...
                self.timer2.stop()
            plotter.plot_data(canvas)
        except Exception as e:
            print(f"You Didn't Start Reading To Stop: {e}")

    def update_plot(self, plotter, canvas):
        plotter.plot_data(canvas)
    #######################################################################
